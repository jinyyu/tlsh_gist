...
Datum tlsh_out(PG_FUNCTION_ARGS)
{
	...
	lsh_bin tmp;
	for (int k = 0; k < TLSH_CHECKSUM_LEN; k++) {    
      tmp.checksum[k] = swap_byte( lsh->checksum[k] );
    }
    tmp.Lvalue = swap_byte( lsh->Lvalue );
    tmp.Q.QB = swap_byte( lsh->Q.QB );
    for( int i=0; i < CODE_SIZE; i++ ){
        tmp.tmp_code[i] = (lsh->tmp_code[CODE_SIZE-1-i]);
    }

	char *str = (char *)palloc(TLSH_HASH_LENGTH + 1);
	to_hex((uint8 *)&tmp, TLSH_INTERNAL_LENGTH, str);
